...
export const EntrevistaReportGenerator: React.FC<EntrevistaReportGeneratorProps> = ({ entrevista }) => {
  ...
  const generarPDF = () => {
    ...
    // COMENTARIOS
    const campos = ['texto_0', 'texto_1', 'texto_2', 'texto_3', 'texto_4', 
                    'comentarios_0', 'comentarios_1', 'comentarios_2', 'comentarios_3', 'comentarios_4'];
    
    const comentarios = campos.filter(c => entrevista[c]);
    ...
    if (comentarios.length > 0) {
      ...
      comentarios.forEach((campo) => {
        ...
        doc.setFontSize(11);
        doc.setFont('helvetica', 'bold');
        const titulo = campo.includes('texto') ? 'Texto' : 'Comentario';
        const numero = campo.match(/\d+/)?.[0] || '0';
        doc.text(`${titulo} ${numero}:`, margin, yPosition);
        yPosition += 8;

        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        const lineas = splitText(entrevista[campo], maxWidth);
